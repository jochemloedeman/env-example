...
def walk_project(
    ...
    def walk_dir(dir: Path, parent_package: str) -> Iterator[ModuleContext]:
        is_package = False
        ...
        for item in dir.iterdir():
            if item.is_file() and item.suffix == ".py":
                yield from extract_module_contexts(
                    module_content=item.read_text(),
                    package=new_parent,
                )
            if (
                item.is_dir()
...
def run(
    ...
    settings = extract_settings(module_contexts=contexts)
